...
Definition f6' (n : nat) : val :=
  λ: "l",
    ...
    let: "p" := NewProph in
    let: "backup" := !("l" +ₗ #1) in
    if: bff3_valid "backup" && (Resolve !"l" "p" #() = "ver") then (
      ("data", "backup", "ver")
    ) else (
      let: "backup'" := !("l" +ₗ #1) in
...
Class fe19 (Σ : gFunctors) := {
  ...
  b134 :: inG Σ b15;
  ea135 :: mono_natG Σ;
  f136 :: ghost_varG Σ bool;
  d137 :: ghost_varG Σ (loc * list val);
  da138 :: tokenG Σ;
  a139 :: inG Σ c17;
  adf140 :: inG Σ cfb18;
  bf141 :: inG Σ bbf16;
...
Admitted.
  ...
  Definition c111 (vs : list (val * val)) : option Z :=
    match vs with
...
Admitted.
  ...
  Lemma d113 γ (lactual lactual' : loc) (actual actual' : list val) requests (x10 : gmap loc (gname * list val)) (γₜ : gname) :
    length actual > 0 →
  ...
  Lemma a115 (dq : dfrac) (γ : gname) (m : gmap loc (agree (gname * list val))) :
  ...
  Lemma a115' (dq : dfrac) (γ : gname) (m : gmap loc (agree nat)) :
    own γ (●{dq} m) ==∗ own γ (●{dq} m) ∗ own γ (◯ m).
  Proof.
    iIntros "H●".
    iMod (own_update with "H●") as "[H● H◯]".
  ...
  Lemma a115'' (dq : dfrac) (γ : gname) (m : gmap nat (agree loc)) :
    own γ (●{dq} m) ==∗ own γ (●{dq} m) ∗ own γ (◯ m).
  Proof.
    iIntros "H●".
    iMod (own_update with "H●") as "[H● H◯]".
  ...
  Lemma x118 `{Countable K} {V} (m m' : gmap K V) P :
  ...
  Lemma f119 k : Nat.even (S k) = negb (Nat.even k).
  Proof.
...
Lemma x120_ne {A} (l : list A) i j x y :
  NoDup l →
  i ≠ j →
  l !! i = Some x →
  l !! j = Some y →
...
Qed.
  ...
  Lemma eb121 n : Nat.odd n = negb (Nat.even n).
  Proof.
    by apply be101.
  Qed.

  Lemma d122 {A} (R : A → A → Prop) l i j x y :
    StronglySorted R l →
    i < j →
    l !! i = Some x →
    l !! j = Some y →
  ...
  Lemma x123 Φ γ γₗ γₑ γᵣ γₜ (backup lexp lexp' ldes : loc) expected desired actual (dq dq' : dfrac) i used :
    lexp' ≠ backup →
  ...
  Global Instance ad124`{EqDecision K, Countable K} `{V} : FMap (gmap K).
  Proof. apply _. Qed.

  Lemma fcc125 (γ γᵥ γₕ γᵣ γᵢ γ_val γ_vers γₒ γₚ' : gname) (l ldes ldes' : loc) (dq : dfrac)
  ...
  Proof.
    ...
    - rewrite Zrem_even x103 Heven /=.
      ...
      { iExFalso.
        ...
        eapply d122 with (i := S (Nat.div2 ver)) (j := S (S (Nat.div2 ver₄))) in Hmono₅; eauto; last lia.
      ...
      iModIntro.
      iApply ("HΦ" with "[$]").
    - rewrite Zrem_odd fbe104 eb121 Heven /=.
      destruct ver as [|ver].
      { done. }
      simpl. wp_pures.
      iApply ("HΦ" with "[$]").
  ...
  Lemma cd126 `{!invGS Σ} (E1 E2 E3 : coPset) (P : iProp Σ) :
    E2 ⊆ E3 →
  ...
  Proof.
    ...
    iMod (d113 with "Hlogtokens Hγ Hlft") as "(Hlogtokens & Hγ & Hlft)".
    { done. }
    { done. }
    { done. }
    { done. }
    { by destruct (log₁ !! ldes'). }
    iMod (d113 with "Hlogtokens Hγ Hrht") as "(Hlogtokens & [Hγ Hγ'] & Hrht)".
    { done. }
    { done. }
    { done. }
    { done. }
    ...
    iMod (a115 with "●Hγₕ") as "[●Hγₕ ◯Hγₕcopy]".
    ...
    iPoseProof (e114_update with "Hlogtokens Hγₚ' Hldes'") as "Hlogtokens".
    { done. }
